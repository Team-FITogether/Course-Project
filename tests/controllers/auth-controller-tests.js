...
describe("registerUser() tests", () => {
    ...
    it("data.createUser() should be called once if username is not existing in the database", () => {
        let dataMock = {
            getUserByUsername() {
                return new Promise(resolve => resolve());
            },
        ...
        return controller.registerUser(reqMock, resMock)
            .then(() => {
                expect(dataSpy.calledOnce).to.be.true;
                dataSpy.restore();
            });
    ...
    it("data.createUser() should not be called if username is existing in the database", () => {
        let foundUser = {
            username: "newUser"
        };
        let dataMock = {
